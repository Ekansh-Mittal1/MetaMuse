...
class TestNormalizerIntegration:
    ...
    def test_get_available_ontologies_real(self):
        ...
        expected_ontologies = ["mondo", "efo", "pato", "uberon", "hancestro", "hsapdv", "dron", "clo"]
    ...
    @pytest.mark.skipif(
        not any([
            (Path(__file__).parent.parent / "src" / "normalization" / "dictionaries" / f"{ont}_terms.json").exists() 
    ...
    def test_semantic_search_with_real_ontology(self):
        ...
        if available_ontology:
            ...
            try:
                results = semantic_search_ontology("diabetes", available_ontology, top_k=3, min_score=0.3)
                ...
                print(f"✅ Semantic search test passed with {available_ontology}: found {len(results)} results")
                
            except Exception as e:
                pytest.skip(f"Semantic search failed with {available_ontology}: {str(e)}")
        else:
    ...
    def test_normalize_candidates_file_integration(self):
        ...
        candidates_file = self.session_dir / "GSE29282" / "GSM1000981_disease_candidates.json"
        ...
        ontologies = get_available_ontologies()
        available_ontologies = [name for name, info in ontologies.items() if info["available"]]
        ...
        try:
            ...
            total_normalized = (
                len(result.normalized_sample_candidates) +
                len(result.normalized_abstract_candidates)
            )
            ...
            pytest.skip(f"Normalization failed (expected with missing ontologies): {str(e)}")
    ...
    def test_batch_normalization_integration(self):
        ...
        try:
            ...
            if result.get("success"):
                ...
                print(f"✅ Batch normalization completed: {batch_data.get('total_candidates_normalized', 0)} candidates processed")
            else:
    ...
    def test_ontology_field_mapping_integration(self):
        ...
        assert ontology_tool is not None, "Get available ontologies tool should be available"
...
class TestNormalizerRealism:
    ...
    def test_common_disease_terms(self):
        ...
        for disease in common_diseases:
            for ontology in available_ontologies[:2]:  # Limit to first 2 to avoid long tests
                try:
                    results = semantic_search_ontology(disease, ontology, top_k=3, min_score=0.3)
                    
                    # Should find some results for common diseases
                    if results:
                        print(f"✅ Found {len(results)} matches for '{disease}' in {ontology}")
                        for result in results:
                            assert result.score >= 0.3
    ...
    def test_abbreviation_expansion(self):
        ...
        for abbrev in abbreviations:
            for ontology in available_ontologies[:1]:  # Test with first available ontology
                try:
                    results = semantic_search_ontology(abbrev, ontology, top_k=5, min_score=0.2)
                    
                    if results:
                        print(f"✅ Found {len(results)} matches for abbreviation '{abbrev}' in {ontology}")
                        ...
                        if high_confidence_matches:
                            print(f"   High confidence match: {high_confidence_matches[0].term} ({high_confidence_matches[0].score:.3f})")
    ...
    def test_ontology_coverage(self):
        ...
        for term, expected_ontology, description in test_cases:
            if expected_ontology in ontologies and ontologies[expected_ontology]["available"]:
                try:
                    results = semantic_search_ontology(term, expected_ontology, top_k=3, min_score=0.3)
                    if results:
                        print(f"✅ {description}: found {len(results)} matches for '{term}' in {expected_ontology}")
                except NormalizationError:
