...
def annotate_celltype_scRNA(
    adata_filename, data_dir, data_info, 
    cluster='leiden', llm='claude-3-5-sonnet-20241022', composition=None, 
...
def get_uce_embeddings_scRNA(adata_filename, data_dir, DATA_ROOT = "/dfs/project/bioagentos/data/singlecell/", custom_args=None):
    """
...
"""
    ...
    try:
        ...
        steps.append(f"Please install the UCE package first. Follow https://github.com/snap-stanford/UCE.git.")
        return "\n".join(steps)
...
def map_to_ima_interpret_scRNA(adata_filename, data_dir, custom_args=None):
    ...
    IMA_adata = sc.read_h5ad(f"{DATA_ROOT}/uce_10000_per_dataset_33l_8ep_coarse_ct.h5ad")
    ...
    steps.append(f"Mapped cell types and confidence scores added to adata.obs")
...
def analyze_chromatin_interactions(hic_file_path, regulatory_elements_bed, output_dir="./output"):
    ...
    try:
        ...
        try:
            ...
            if hasattr(c.bins(), 'columns') and 'weight' in c.bins().columns:
                has_weights = True
            elif hasattr(c.bins(), 'keys') and 'weight' in c.bins().keys():
                has_weights = True
            elif hasattr(c, 'bins') and callable(c.bins) and 'bins/weight' in c.bins():
                has_weights = True
        except Exception as e:
    ...
    try:
        reg_elements = pd.read_csv(regulatory_elements_bed, sep='\t', 
    ...
    try:
        ...
        for chrom in c.chromnames:
            ...
            for _, enh in chrom_enhancers.iterrows():
                ...
                for _, prom in chrom_promoters.iterrows():
                    ...
                    if expected > 0:
                        fold_enrichment = interaction_strength / expected
                    else:
                        fold_enrichment = 0
        ...
        if interactions:
            ...
            pd.DataFrame(columns=[
                'chrom', 'enhancer_start', 'enhancer_end', 'enhancer_name',
                'promoter_start', 'promoter_end', 'promoter_name',
                'interaction_strength', 'fold_enrichment'
        ...
        for chrom in c.chromnames:
            ...
            if len(insulation_score) > 0:
                ...
                for i in range(1, len(insulation_score)-1):
                    if (insulation_score[i] < insulation_score[i-1] and 
                        insulation_score[i] < insulation_score[i+1] and
                        insulation_score[i] < -1):  # Threshold for boundary strength
...
def analyze_comparative_genomics_and_haplotypes(sample_fasta_files, reference_genome_path, output_dir="./output"):
    ...
    try:
        ...
        subprocess.run(["bwa"], stderr=subprocess.PIPE)
        
        # Index reference genome
        log.append("Indexing reference genome with BWA...")
        subprocess.run(["bwa", "index", reference_genome_path], 
    ...
    for sample_path in sample_fasta_files:
        ...
        try:
            ...
            subprocess.run(["bwa", "mem", reference_genome_path, sample_path], 
    ...
    shared_variants = defaultdict(list)
    ...
    log.append(f"- Identified variant patterns across samples")
...
def perform_chipseq_peak_calling_with_macs2(chip_seq_file, control_file, output_name="macs2_output", genome_size="hs", q_value=0.05):
    """
    ...
    try:
        check_process = subprocess.run(["macs2", "--version"], 
...
def find_enriched_motifs_with_homer(peak_file, genome="hg38", background_file=None, motif_length="8,10,12", 
                                   output_dir="./homer_motifs", num_motifs=10, threads=4):
    """
    ...
    homer_cmd = [
        ...
        genome,
        output_dir,
        "-size", "200",  # Default search region size (200bp centered on peak)
        "-len", motif_length,
        "-S", str(num_motifs),
        "-p", str(threads)
    ]
    ...
    try:
        ...
        if os.path.exists(known_results):
            ...
            if os.path.exists(known_motifs_txt):
                with open(known_motifs_txt, 'r') as f:
                    lines = f.readlines()
...
def analyze_genomic_region_overlap(region_sets, output_prefix="overlap_analysis"):
    ...
    # Start research log
    log = f"# Genomic Region Overlap Analysis\n"
