...
class LinkerTools:
    ...
    def find_sample_directory(self, sample_id: str) -> LinkerResult:
        ...
        if 'reverse_mapping' in mapping_data and sample_id in mapping_data['reverse_mapping']:
    ...
    def clean_metadata_files(self, sample_id: str, fields_to_remove: List[str] = None) -> LinkerResult:
        """
        ...
        try:
            ...
            if abstract_files:
                ...
                cleaned_abstract_file = cleaned_dir / f"{abstract_file.stem}_cleaned.json"
                self._clean_json_file(abstract_file, cleaned_abstract_file, fields_to_remove)
            ...
            if series_matrix_file.exists():
                cleaned_matrix_file = cleaned_dir / f"{series_id}_series_matrix_cleaned.json"
                self._clean_json_file(series_matrix_file, cleaned_matrix_file, fields_to_remove)
    ...
    def download_series_matrix(self, sample_id: str) -> LinkerResult:
        ...
        if 'file_links' not in matrix_data or not matrix_data['file_links']:
            return LinkerResult(
                success=False,
                message="No file links found in series matrix metadata"
            )
    ...
    def extract_sample_metadata(self, sample_id: str) -> LinkerResult:
        ...
        try:
            ...
            if not data_lines:
                return LinkerResult(
                    success=False,
                    message="No data lines found in series matrix file"
                )
            ...
            for line in data_lines[1:]:  # Skip header line
                if line.strip():
                    ...
                    if len(values) > sample_column:
                        ...
                        sample_value = values[sample_column] if len(values) > sample_column else ""
...
def clean_metadata_files_impl(sample_id: str, session_dir: str, fields_to_remove: List[str] = None) -> Dict[str, Any]:
    """
    Generate cleaned versions of metadata files by removing specified fields.
...
def package_linked_data_impl(sample_id: str, session_dir: str, fields_to_remove: List[str] = None) -> Dict[str, Any]:
    """
    Package all linked information for a sample into a comprehensive result.
