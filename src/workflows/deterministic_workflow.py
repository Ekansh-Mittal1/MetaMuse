...
async def run_deterministic_workflow(
    ...
    try:
        ...
        print(f"✅ Data intake completed successfully")
        print(f"📁 Session directory: {data_intake_output.session_directory}")
        print(f"🔢 Samples for curation: {len(data_intake_output.sample_ids_for_curation)}")
        ...
        print(f"✅ Curator completed successfully")
        print(f"🎯 Target field: {curator_output.target_field}")
        print(f"🔢 Samples processed: {curator_output.total_samples_processed}")
        print(f"🎯 Candidates extracted: {len(curator_output.curation_results) if curator_output.curation_results else 0}")
        ...
        print(f"✅ Normalizer completed successfully")
        ...
        workflow_result = {
            ...
            "files_created": [
                ...
                str(normalizer_file),
                    ...
                    "candidates_extracted": len(curator_output.curation_results) if curator_output.curation_results else 0,
        ...
        return {
            ...
            "error": str(e),
            "session_id": session_id,
            "session_directory": str(Path(sandbox_dir) / session_id) if session_id else None,
            "target_field": target_field,
            "input_text": input_text,
        }
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    ...
                    run_deterministic_workflow(
                        input_text, target_field, session_id, sandbox_dir, ontologies, min_score, model_provider, max_tokens, max_turns
                    )
                )
...
async def test_normalizer_agent(
    ...
    try:
        ...
        print(f"🔄 Overriding session directory from '{curator_output.session_directory}' to '{test_session_dir}'")
        ...
        print(f"📊 Input candidates: {test_result['curator_input']['total_candidates']}")
        print(f"🔬 Normalized results: {test_result['normalizer_output']['sample_results_count']}")
        print(f"✨ Successful normalizations: {test_result['normalizer_output']['successful_normalizations']}")
