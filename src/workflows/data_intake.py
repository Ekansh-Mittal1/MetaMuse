...
class DataIntakeWorkflow:
    ...
    def _validate_inputs(self, geo_ids: Dict[str, List[str]]) -> WorkflowResult:
        ...
        try:
            ...
            for gsm_id in geo_ids['gsm_ids']:
                result = validate_geo_inputs_impl(gsm_id=gsm_id, email=self.email, api_key=self.api_key)
            ...
            for gse_id in geo_ids['gse_ids']:
                result = validate_geo_inputs_impl(gse_id=gse_id, email=self.email, api_key=self.api_key)
                result_data = json.loads(result)
                if result_data['validation_status'] == 'failed':
                    return WorkflowResult(
                        success=False,
            ...
            for pmid in geo_ids['pmid_ids']:
                result = validate_geo_inputs_impl(pmid=pmid, email=self.email, api_key=self.api_key)
                result_data = json.loads(result)
                if result_data['validation_status'] == 'failed':
                    return WorkflowResult(
                        success=False,
    ...
    def _extract_gsm_workflow(self, gsm_id: str) -> WorkflowResult:
        ...
        try:
            ...
            series_result = extract_series_id_from_gsm_metadata_impl(gsm_file, str(self.session_dir))
    ...
    def _link_sample_data(self, sample_id: str, fields_to_remove: List[str] = None) -> WorkflowResult:
        """
        ...
        try:
            ...
            return WorkflowResult(
                ...
                files_created=clean_result.get('files_created', []) + package_result.get('files_created', [])
            )
                ...
                if result.data and 'series_id' in result.data:
                    ...
                    try:
                        ...
                        if mapping_result['success'] and 'data' in mapping_result:
                            ...
                            if 'mapping' in mapping_data and series_id in mapping_data['mapping']:
                                series_sample_ids = mapping_data['mapping'][series_id]['sample_ids']
                                all_sample_ids.extend(series_sample_ids)
                    except Exception as e:
                        print(f"Warning: Could not extract sample IDs from {gse_id}: {e}")
    ...
    def run_linker_workflow(self, sample_ids: List[str], fields_to_remove: List[str] = None) -> WorkflowResult:
        """
        ...
        """
        try:
            print(f"🔧 Starting linker workflow for {len(sample_ids)} samples: {sample_ids}")
    ...
    def run_complete_workflow(self, input_text: str, fields_to_remove: List[str] = None) -> WorkflowResult:
        """
        ...
        try:
            ...
            # Combine results
            all_files_created = (ingestion_result.files_created or []) + (linker_result.files_created or [])
...
Examples:
  ...
  python data_intake.py --input "Extract metadata for GSE41588" --type complete --session my-session --sandbox custom-sandbox
    ...
    parser.add_argument(
        "--session", "-s",
        help="Session ID (generated automatically if not provided)"
    )
