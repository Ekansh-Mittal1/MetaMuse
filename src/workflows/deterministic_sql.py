"""
Deterministic SQL workflow for metadata processing.
...
data between steps. The workflow is: data_intake_sql -> curator -> normalizer.
"""
...
async def run_deterministic_sql_workflow(
    input_text: str,
    ...
    """
    Run the complete deterministic SQL workflow: data_intake_sql -> curator -> normalizer.
    ...
    try:
        ...
        if session_id is None:
            session_id = f"det_sql_workflow_{str(uuid4())}"

        print(f"🚀 Starting deterministic SQL workflow with session: {session_id}")
        ...
        # Stage 1: Data Intake (SQL)
        print("\n" + "=" * 60)
        print("🗂️  STAGE 1: DATA INTAKE (SQL)")
        print("=" * 60)

        data_intake_output = run_data_intake_sql_workflow(
        ...
        print(f"❌ Deterministic SQL workflow failed: {str(e)}")
        import traceback
...
def run_deterministic_sql_workflow_sync(
    input_text: str,
    target_field: str = "Disease",
    session_id: str = None,
    sandbox_dir: str = "sandbox",
    ...
    """
    Synchronous wrapper for the deterministic SQL workflow.

    This function provides a synchronous interface to the async deterministic SQL workflow,
    useful for integration with existing synchronous code.

    Parameters are the same as run_deterministic_sql_workflow.
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run,
                    run_deterministic_sql_workflow(
                        input_text,
                        target_field,
                        session_id,
                        sandbox_dir,
