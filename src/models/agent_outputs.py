...
class LinkerOutput(BaseModel):
    ...
    recommended_curation_fields: List[str] = Field(
        default_factory=list, description="Recommended target fields for curation"
    )
...
class CuratorOutput(BaseModel):
    ...
    def validate_average_confidence(cls, v):
        ...
        if isinstance(v, str):
            # Handle various "not applicable" variations
            if any(phrase in v.lower() for phrase in ["not applicable", "n/a", "none", "no candidates", "candidates were found"]):
                return None
        return v
...
class SampleTypeCuratorOutput(BaseModel):
    """Structured output from CuratorAgent for SampleType target field."""
    ...
    target_field: str = Field(
        default="SampleType", description="Target metadata field that was curated"
    )
    session_directory: str = Field(..., description="Session directory used")

    # Output data - full SampleTypeCurationResult objects with complete information
    curation_results: Optional[List[SampleTypeCurationResult]] = Field(
        default=None,
        description="Detailed curation results with full SampleTypeCurationResult objects including enum classification and confidence scores",
    )
    ...
    primary_samples_count: int = Field(
        ..., ge=0, description="Number of samples classified as primary (patient biopsy)"
    )
    cell_line_samples_count: int = Field(
        ..., ge=0, description="Number of samples classified as cell lines"
    )
    unknown_samples_count: int = Field(
        ..., ge=0, description="Number of samples classified as unknown"
    )
    samples_needing_review: int = Field(
        ..., ge=0, description="Samples requiring manual review"
    )
...
class AssayTypeCuratorOutput(BaseModel):
    """Structured output from CuratorAgent for AssayType target field."""
    ...
    target_field: str = Field(
        default="AssayType", description="Target metadata field that was curated"
    )
    session_directory: str = Field(..., description="Session directory used")

    # Output data - full AssayTypeCurationResult objects with complete information
    curation_results: Optional[List[AssayTypeCurationResult]] = Field(
        default=None,
        description="Detailed curation results with full AssayTypeCurationResult objects including enum classification and confidence scores",
    )
    ...
    single_cell_samples_count: int = Field(
        ..., ge=0, description="Number of samples classified as single cell"
    )
    bulk_samples_count: int = Field(
        ..., ge=0, description="Number of samples classified as bulk"
    )
    unknown_samples_count: int = Field(
        ..., ge=0, description="Number of samples classified as unknown"
    )
...
class DiseaseCuratorOutput(BaseModel):
    """Structured output from CuratorAgent for Disease target field."""
    ...
    target_field: str = Field(
        default="Disease", description="Target metadata field that was curated"
    )
    session_directory: str = Field(..., description="Session directory used")

    # Output data - full DiseaseCurationResult objects with complete information
    curation_results: Optional[List[DiseaseCurationResult]] = Field(
        default=None,
        description="Detailed curation results with full DiseaseCurationResult objects including disease name, condition, and confidence scores",
    )
    ...
    control_samples_count: int = Field(
        ..., ge=0, description="Number of samples classified as control"
    )
