...
from .result_models import AgentResult
from .agent_outputs import IngestionOutput, LinkerOutput, CuratorOutput
...
class ModelSerializer:
    ...
    def serialize_metadata(
        ...
        try:
            ...
            raise SerializationError(
                f"Failed to serialize metadata to {file_path}: {str(e)}"
            )

    def serialize_mapping(
        self, mapping: SeriesSampleMapping, filename: str = "series_sample_mapping.json"
    ) -> str:
        """
        ...
        try:
            ...
            raise SerializationError(
                f"Failed to serialize mapping to {file_path}: {str(e)}"
            )

    def serialize_linked_data(
        self, linked_data: LinkedData, filename: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            raise SerializationError(
                f"Failed to serialize linked data to {file_path}: {str(e)}"
            )

    def serialize_agent_result(
        self, result: AgentResult, filename: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            raise SerializationError(
                f"Failed to serialize result to {file_path}: {str(e)}"
            )
    ...
    def serialize_agent_output(
        ...
        try:
            ...
            raise SerializationError(
                f"Failed to serialize output to {file_path}: {str(e)}"
            )
    ...
    def serialize_cleaned_metadata(
        ...
        try:
            ...
            raise SerializationError(
                f"Failed to serialize cleaned metadata to {file_path}: {str(e)}"
            )

    def serialize_cleaned_metadata_models(
        self,
        cleaned_models: Dict[
            str,
            Union[
                CleanedSeriesMetadata, CleanedSampleMetadata, CleanedAbstractMetadata
            ],
        ],
        series_id: str,
    ) -> List[str]:
        """
        Serialize cleaned metadata models to JSON files.

        Parameters
        ----------
        cleaned_models : Dict[str, Union[CleanedSeriesMetadata, CleanedSampleMetadata, CleanedAbstractMetadata]]
            Dictionary of cleaned metadata models by type
        series_id : str
            Series ID for organizing files
        ...
        # Ensure cleaned subdirectory exists
        cleaned_dir = series_dir / "cleaned"
        cleaned_dir.mkdir(exist_ok=True)

        files_created = []

        for model_type, model in cleaned_models.items():
            try:
                if isinstance(model, CleanedSeriesMetadata):
                    filename = f"{model.series_id}_metadata_cleaned.json"
                elif isinstance(model, CleanedSampleMetadata):
                    filename = f"{model.sample_id}_metadata_cleaned.json"
                elif isinstance(model, CleanedAbstractMetadata):
                    filename = f"PMID_{model.pmid}_metadata_cleaned.json"
                else:
                    raise SerializationError(
                        f"Unknown cleaned metadata type: {type(model)}"
                    )
                ...
                raise SerializationError(
                    f"Failed to serialize {model_type} cleaned metadata: {str(e)}"
                )
...
class WorkflowSerializer:
        ...
        for sample_id, metadata_list in cleaned_metadata.items():
            for metadata in metadata_list:
                if isinstance(metadata, GSMMetadata):
                    ...
                    series_id = (
                        linked_data_objects.get(sample_id, {}).series_id
                        if sample_id in linked_data_objects
                        else None
                    )
                    files_created.append(
                        self.serializer.serialize_cleaned_metadata(
    ...
    def serialize_curator_workflow(self, output: CuratorOutput) -> List[str]:
        """
...
def load_metadata_from_json(
    file_path: Union[str, Path], metadata_type: str
