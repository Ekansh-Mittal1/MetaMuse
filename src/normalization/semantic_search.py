...
class OntologySemanticSearch:
    ...
    def __init__(self, dictionary_path, model_name="cambridgeltl/SapBERT-from-PubMedBERT-fulltext"):
        """
    ...
    def save_index(self, output_dir="src/normalization/semantic_indexes"):
        ...
        if self.use_gpu_faiss and hasattr(self.index, 'index'):
            ...
            index_path = Path(output_dir) / f"{Path(self.dictionary_path).stem}_faiss_gpu.index"
            faiss.write_index(cpu_index, str(index_path))
        else:
            # CPU index can be saved directly
            index_path = Path(output_dir) / f"{Path(self.dictionary_path).stem}_faiss_cpu.index"
    ...
    def load_index(self, output_dir="src/normalization/semantic_indexes"):
        ...
        gpu_index_path = Path(output_dir) / f"{Path(self.dictionary_path).stem}_faiss_gpu.index"
        cpu_index_path = Path(output_dir) / f"{Path(self.dictionary_path).stem}_faiss_cpu.index"
        metadata_path = Path(output_dir) / f"{Path(self.dictionary_path).stem}_metadata.pkl"
